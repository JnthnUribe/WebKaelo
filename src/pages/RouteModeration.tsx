import { pendingRoutes as mockPendingRoutes } from "@/lib/mockData";
import { fetchPendingRoutes, approveRoute, rejectRoute } from "@/lib/supabaseService";
...
export default function RouteModeration() {
  ...
  const [routes, setRoutes] = useState<any[]>(isDemoMode ? mockPendingRoutes : []);
  const [loadingId, setLoadingId] = useState<string | null>(null);

  useEffect(() => {
    if (isDemoMode) { setRoutes(mockPendingRoutes); return; }
    fetchPendingRoutes().then(setRoutes);
  }, [isDemoMode]);

  const handleAction = async (id: string, action: "approve" | "reject") => {
    setLoadingId(id);
    const result = action === "approve" ? await approveRoute(id) : await rejectRoute(id);
    setLoadingId(null);
    ...
    if (result.error) {
      ...
      toast.success(action === "approve" ? "Ruta aprobada y publicada" : "Ruta rechazada");
    }
  };

  return (
    <div className="space-y-5 animate-fade-in">
      <h1 className="text-2xl font-bold">Moderacion de Rutas</h1>
      {routes.length === 0 ? (
        <div className="text-center py-16"><p className="text-4xl mb-3">✅</p><p className="text-muted-foreground">No hay rutas pendientes de revision</p></div>
      ) : (
